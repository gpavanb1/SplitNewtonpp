...
TEST(NewtonTest, SimpleConvergence)
{
    ...
    auto [x, step, iter, status] = newton(df, J, x0, 1, false, 2.0, 2.0, false, std::nullopt, 0.8);
...
TEST(NewtonTest, RosenbrockConvergence)
{
    ...
    auto [x, s, iter, status] = newton(df, J, x0, 100, false, 0.0, 1.0, false, std::nullopt, 0.8, 1);
...
TEST(NewtonTest, ArmijoRule)
{
    ...
    auto [x, step, iter, status] = newton(df, J, x0, 1, false, 0.1, 1.0, true, std::nullopt, 0.8);
